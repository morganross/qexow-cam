...
const checks = [
  ["package version is 2.1.50", pkg.version === "2.1.50"],
  ...
  ["daemon exposes CAM_VERSION 2.1.50", daemon.includes('const CAM_VERSION = "2.1.50";')],
  ...
  ["peer sync preserves prior mirrors on transient empty remote export", daemon.includes("peer.sync.empty_inventory_preserved") && daemon.includes("remote-export-returned-zero-agents-and-zero-discoveries-after-prior-success")],
  ...
  ["CLI inventory export refreshes local discovery by default", cli.includes("cam inventory export [--no-refresh]") && cli.includes("refreshLocalRegistryFromThreads") && cli.includes("discoverThreads()")],
  ["CLI rollout verification command exists", cli.includes("cam verify-rollout") && cli.includes("rollout verification failed") && cli.includes('if (cmd === "verify-rollout") return commandVerifyRollout();')],
  ...
  ["release workflow selects preserve-state task correctly", workflow.includes('"/TASKS=preserve_state"')],
